...
<style type="text/css"> 
    ...
    .thrColFixHdr #mainContent { 
    	margin: 0 100px; /* los márgenes derecho e izquierdo de este elemento div crean las dos columnas externas de los lados de la página. Con independencia de la cantidad de contenido que incluyan los divs de las barras laterales, permanecerá el espacio de la columna. Puede quitar este margen si desea que el texto del div #mainContent llene el espacio de las barras laterales cuando termine el contenido de cada una de ellas. */
    	padding: 0 5px; /* recuerde que el relleno es el espacio situado dentro del cuadro div y que el margen es el espacio situado fuera del cuadro div */
    } 
    .thrColFixHdr #footer { 
...
<a href="http://www.google.com.bo"><img src="Img/google-logo.gif" width="150" height="60" border="0" /></a><br />
      ...
      <div id="mainContent">
      <h3>Lista de Padres de familia registrados en la base de datos APF</h3>
      <?php
      if (isset($_SESSION['alias']))
	  {
	    $query1="SELECT * FROM registrados";
		$resultado1=mysql_query($query1) or die(mysql_error());
		$total1= mysql_num_rows($resultado1);
		$var=1;
		if($total1>0){
		  ?>
		  <table width="651" border="1" bordercolor="#999999">
		  <tr>
		   <td width="16"><span class="Estilo6">#</span></span></td>
		   <td width="146"><span class="Estilo6">Nombre Padre</span></td>
		   <td width="125"><span class="Estilo6">Nombre Madre</span></td>
		   <td width="128"><span class="Estilo6">Nombre Estudiante</span></td>
		   <td width="60" ><span class="Estilo6">Curso</span></td>
		   <td width="150"><span class="Estilo6">Correo de contacto</span></span></td>
		 </tr>
		 <?php 
		 while($fila=mysql_fetch_assoc($resultado1)){
		 echo "<tr>";
		   echo '<td ><span class="Estilo1">'.$var.'</span></span></td>';
		   echo '<td ><span class="Estilo1">'.$fila['nombrepadre'].'</span></td>';
		   echo '<td ><span class="Estilo1">'.$fila['nombremadre'].'</span></td>';
		   echo '<td ><span class="Estilo1">'.$fila['nombreestudiante'].'</span></td>';
		   echo '<td ><span class="Estilo1">'.$fila['curso'].'</span></td>';
		   echo '<td ><span class="Estilo1">'.$fila['email'].'</span></span></td>';
		 echo '</tr>';
		 $var=$var+1;
	 	}?>
       </table>
